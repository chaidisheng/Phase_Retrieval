...
def prox_fn_sd(tensor, *args, **kwargs):
    ...
    x = torch.tensor(kwargs.get('x', None))  # unknown image
...
def prox_gn(tensor, mu, lambd, denoiser):
    # type: (Tensor, float, list, list) -> Tensor
    ...
    # p = np.random.choice((-1, 1), size=1, replace=True, p=(0.1, 0.9))
    # if p == -1:
    #     v = denoise(tensor, sigma[0], blind=False, denoise_method=denoiser)
    # else:
    #     v = denoise(tensor, sigma[1], blind=False, denoise_method=denoiser)
    p = np.random.choice(len(lambd), size=1, replace=True, p=(0.5, 0.5))
    v = denoise(tensor, sigma[int(p)], blind=True, denoise_method=denoiser[int(p)])
    return v


def PnP_Stochastic_ADMM(y, ForwardFunc, BackwardFunc, denoiser, InitEst, **options):
    # type: (Tensor, object, object, list, Tensor, dict) -> Tensor
