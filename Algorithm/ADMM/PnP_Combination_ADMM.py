...
def prox_gn(tensor, mu, lambd, denoiser):
    ...
    p = [0.1, 0.9]
    # p = [list(range(1, len(lambd) + 1))[i]/sum(range(1, len(lambd) + 1)) for i in range(len(lambd))]
    v = sum([p[i] * denoise(tensor, sigma[i], blind=False, denoise_method=denoiser) for i in range(len(lambd))])
    return v


def PnP_Combination_ADMM(y, ForwardFunc, BackwardFunc, denoiser, InitEst, **options):
