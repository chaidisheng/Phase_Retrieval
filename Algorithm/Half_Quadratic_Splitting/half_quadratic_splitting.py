...
from Algorithm.Convex_Optimization.HQS.half_quadratic_splitting import solve
...
def PnP_HQS(y, ForwardFunc, BackwardFunc, denoiser, InitEst, **options):
...
def half_quadratic_splitting_(y, ForwardFunc, BackwardFunc, denoiser, InitEst, **options):
    """TODO: Docstring for PnP_ADMM.

    optional problem: minimzie f(Ax) + g(x)
    :arg1: ForwardFunc operator
    :arg2: BackwardFunc operator
    :arg3: f objective function
    :arg4: subgradient of f
    :arg5: options dictionary
    :returns: restoration image
    ...
    while(residual[-1] > tol and iter <= maxiter):
        ...
        for inner_iter in range(inner_iters):
            ...
            if sub_method is 'SD':
                ...
                v = ForwardFunc(x)
                abs_v = torch.sqrt(torch.sum(v**2, 3, keepdim=True))
                subgradient = 1./sigma_w**2*BackwardFunc(v - y*(v/abs_v)) + rho*(x - z)
                x = x - mu*subgradient
            elif sub_method is 'PGM':
                v = ForwardFunc(x)
                abs_v = torch.sqrt(torch.sum(v**2, 3, keepdim=True))
                subgradient = BackwardFunc(v - y*(v/abs_v))
                x = (rho*mu*z + x - mu*subgradient)/(1.0 + rho*mu)
            else: # the fixed point
                print('sub_method is %s.' % sub_method)
                v = ForwardFunc(x)
                abs_v = torch.sqrt(torch.sum(v**2, 3, keepdim=True))
                x = (1 - beta)*x + beta*(BackwardFunc(y*(v/abs_v)) + rho*z)/(1 + rho)
        ...
        residual.append(residualx + residualz)

        if verbose:
            print("%3g \t %3.5e \t %3.5e \n" % (iter, residualx, residualz))
