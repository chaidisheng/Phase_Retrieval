...
def set_defaults(A, At, x0, gradf, options):
    r"""TODO: Docstring for setdefaults: Fill in the dictionary of options with the default values.
    :returns: options
    """
...
def fasta(A, At, f, gradf, g, proxg, x0, **options):
   ...
   Outputs:
    ...
    """
    # global variables
    global solution, output, best_objective_iterate, x_accel1, d_accel1, alpha1
    ...
    for iters in range(max_iters):
        ...
        if options["verbose"] and backtrack_count > 10:
            print("{0}\tWARNING: excessive backtracking {1} steps, current stepsize is {2}\n".format
                  (options["string_header"], backtrack_count, tau0))
        ...
        normalized_residual.append(residual[iters] / normalizer) # It has problem
        fvals.append(f1)
        ...
        if options["verbose"]:
            print("%s%d: residual = %0.2d, backtrack = %d, tau = %d" %
                  (options["string_header"], iters, residual[iters], backtrack_count, tau0))  # format
            if options["record_objective"]:
        ...
        if options["accelerate"]:
            ...
            alpha1 = (1. + torch.tensor(np.sqrt(1 + 4.*alpha0**2)))/2
            # over relax/predict
